    ...
    public class HomeController : Controller
    {
        public void dump_news_info_excel(string link, List<ArticleCriteria> articleList)
        {
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                FileInfo excelFile = new FileInfo(@"C:\Users\Admin\Desktop\ADO.NET\NetProject-University-master\.NetProject-University-master\NetProject__UNIVERSITY_\NetProject__UNIVERSITY_\excels\test.xlsx");
        ...
        public IActionResult Index()
        {
            ...
            //var publicationNumber = Regex.Matches(publicationNode.InnerHtml, "<li>").Count;
            //var hyperlinks = Regex.Matches(publicationNode.InnerHtml, "href=\\\"(.*?)\"");
            //var test = hyperlinks[0].Groups[1].Value;
            //var publications = publicationNode.SelectNodes(publicationNode.XPath +"//li");
            //var hyperlink = publicationNode.ChildNodes;
            //var dummy = 1;
            //using (university_projectContext db = new university_projectContext())
            //{
            //    var lecturer = db.Lecturers.First();
            //}
            //var html = @"http://ami.lnu.edu.ua/academics/bachelor/curriculum-applied-mathematics";
            //HtmlWeb web = new HtmlWeb();
            //var htmlDoc = web.Load(html);
            //var publicationNode = htmlDoc.DocumentNode.SelectNodes("//table//tbody/tr/td[@class='title']").First();
            //var publicationNumber = Regex.Matches(publicationNode.InnerHtml, "<li>").Count;

            //var html = @"http://ami.lnu.edu.ua/course/numerical-methods-informatics";
            //HtmlWeb web = new HtmlWeb();
            //var htmlDoc = web.Load(html);
            //var publicationNode = htmlDoc.DocumentNode.SelectNodes("//section[@class='attachments']//a");
            ...
            //using (university_projectContext db = new university_projectContext())
            //{
            //    course = db.Courses.Where(md => md.CourseId == 4224).First();
            //}

            //var test = Regex.Matches(course.Materials, "href=\\\"(.*?)\"");
            ...
            //var test1 = Regex.Matches(publicationNode.InnerHtml, "href=\\\"(.*?)\"");
            //var test = Regex.Matches(publicationNode.InnerHtml, "href=\\\"(.*?)\"").Select(hp => hp.Groups[1].Value);

            var markingDate = new MarkingDate();
            //markingDate.Date = DateTime.Today;
            using (university_projectContext db = new university_projectContext())
            {
                //db.MarkingDate.Add(markingDate);
                //db.SaveChanges();
                markingDate = db.MarkingDate.Where(md => md.DateId == 24).First();
            }
            MainClass.MainCollecting(markingDate);
            DataAnalyser.MainAnalyse(markingDate);
            //HtmlWeb web = new HtmlWeb();
            //var lecturerPage = web.Load("http://physics.lnu.edu.ua/employee/yakibchuk").DocumentNode;
            //var test = DataCollector.GetAllInfoRows(lecturerPage);
        ...
        public FileStreamResult GetMaterialsbyFaculties(MarkingDate markingDate)
        {
            var memoryStream = DataWriterExcels.WriteExcelMaterials1(markingDate);
            return File(memoryStream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        }

        public FileStreamResult GetCoursesbyFaculties(MarkingDate markingDate)
        {
            var memoryStream = DataWriterExcels.WriteExcelCourses1(markingDate);
            return File(memoryStream, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        }
